    ...
    ngOnInit(): void {
        ...
        if (this.field == 'IntBattVolt')
        {
            if (this.dataService.deviceData[this.idDevice].type == 'to-mslr') {
                ...
                this.alarmLineLow.push({'date':this.dataService.historyIntervalData[5].dateInterval.start, 'alarmValue': 3.27});
                this.alarmLineLow.push({'date':this.dataService.historyIntervalData[0].dateInterval.stop, 'alarmValue': 3.27});
            }
        }
    }
